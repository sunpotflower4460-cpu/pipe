...
- GET / または GET /admin
  - 「ファイルを追加」（ZIP）と「シェア」を主導線にした Code Memo UI
  - ZIP追加後のファイル一覧と、行番号付きコード本文の表示
  - フォルダ作成・フォルダ単位表示・フォルダ単位シェアURLコピー
  - 複数ファイル選択で「AIに見せるセット」を作成し、共有対象外パスを明示
  - ファイル単位の隠す / 戻す / 削除
...
GET /t/{token}/share/{share_id}/index
フォルダ共有または「AIに見せるセット」共有のインデックスを返す。

GET /t/{token}/share/{share_id}/file?path=&from=&to=
共有範囲内のファイルだけを行番号付きで返す（範囲外は 404）。
