...
実装・改修の際、以下4点は例外なく守ること。レビューでもここを最初に確認する。
...
5. GET /index
6. GET /file（行範囲・行番号・上限・続き案内）★最重要
...
READMEは以上です。リポジトリ直下に README.md として置けば、エージェントが作業のたびに参照でき、鉄則・仕様・受け入れ条件がぶれにくくなります。特に冒頭の「絶対に守る鉄則」4点と、末尾の「受け入れ条件」が、実装がずれたときに立ち返る基準になります。
