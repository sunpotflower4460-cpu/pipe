...
async function renderHome(request: Request, env: Env, notice = ""): Promise<string> {
  ...
  return page(appName, `
    ...
    <section class="card"><h2>ファイルを追加</h2><form method="post" enctype="multipart/form-data" action="/memos/create"><label>メモ名<input name="name" value="Code Memo"></label><br><br><label>ZIPファイル<input type="file" name="file" accept=".zip,application/zip" required></label><br><br><div class="row"><button>ファイルを追加</button><a class="button" href="#share">シェア</a></div></form></section>
