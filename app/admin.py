...
def _render_admin_page(request: Request, error_message: str | None = None) -> HTMLResponse:
    ...
    for token, record in records:
        ...
        rows.append(
            "<li>"
            f"<h3>{escape(name)}</h3>"
            f"<div>source_type: {escape(source_type)}</div>"
            f"{repository_label}"
            f"<div>created_at: {_display_time(record.get('created_at'))}</div>"
            f"<div>expires_at: {_display_time(record.get('expires_at'))}</div>"
            f"<div>status: {escape(state)}</div>"
            f"<div>files: {total_files}</div>"
            f"<div>total_lines: {total_lines}</div>"
            "<div>まずClaudeには index URL を渡してください。特定のファイルを読ませたい時は file URL を使います。レビューが終わったら revoke で止められます。</div>"
            f"<div>index URL: <input type='text' readonly value='{escape(index_url)}' size='96'></div>"
            f"<div>file URL 例: <input type='text' readonly value='{escape(file_url)}' size='96'></div>"
    ...
    body = (
        ...
        "<p>このアプリは、ClaudeやChatGPTにコードを読んでもらうための安全な中継パイプです。"
        ...
        "<div><label>repository_url: <input type='url' name='repository_url' placeholder='https://github.com/owner/repo.git'></label></div>"
        "<div><label>read-only token（任意）: <input type='password' name='access_token'></label>"
        "<small> この入力は保存・表示・ログ出力しません。</small></div>"
        "<div><label>ZIPファイル: <input type='file' name='file' accept='.zip,application/zip'></label></div>"
